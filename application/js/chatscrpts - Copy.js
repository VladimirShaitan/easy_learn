...
var base = 'http://localhost/opskill/'; 
...
$('.chat-floting-btn') .click(function () {
 	$('#chat-container').toggle('slide', {
        direction: 'right'
    }, 500);
...
$(document).on('click', '[data-toggle="popover"]', function(){  
		...
		if($('#chat-box-'+user).is(':visible')){
			...
			// 
				...
				$('#chat-box-'+user+' .chat-box-body').slimScroll({ scrollBy: '300px' });
...
$(document).on('keypress', '.chat-textarea input', function(e){
		...
		var userID = $(this).next('#chat_buddy_id').val();
			...
			cache: false,
                success: function(response){
                    ...
                    $('div#chat-box-'+userID+' ul#chat_box_body_id-'+userID).append(li);

                    $('div#chat-box-'+userID+' ul#chat_box_body_id-'+userID).animate({scrollTop: $('div#chat-box-'+userID+' ul#chat_box_body_id-'+userID).prop("scrollHeight")}, 500);
                }
            });
        }
});
                                ...
                                if(this.sender == chatbuddy){
                                  li = '<li class="'+ this.type +'">\
 								  <img src="'+base_url+'uploads/profiles/'+this.avatar+'" class="avt img-responsive">\
                                    ...
                                    $('div#chat-box-'+chatbuddy+' ul#chat_box_body_id-'+chatbuddy).append(li);
                                    $('div#chat-box-'+chatbuddy+' ul#chat_box_body_id-'+chatbuddy).animate({scrollTop: $('div#chat-box-'+chatbuddy+' ul#chat_box_body_id-'+chatbuddy).prop("scrollHeight")}, 500);
                                    ...
                                    $.each(senders, function() {
                                        if(this.user == from){
											...
											if(currntTitle.indexOf('(') === -1){
											 ...
											 $('div#chat-box-'+from+'.chat-box div.chat-box-header span.tinyMsgCount').html(this.count);
											 $('div#chat-box-'+from+'.chat-box div.chat-box-header span.tinyMsgCount').show();
											//tinyMsgCount
											//a//lert(2);
                            ...
                            $.each(senders, function() {
                                if(this.user == from){
									...
									//alert(3);
									 ...
									 $('div#chat-box-'+this.sender+' .chat-box-header span.tinyMsgCount').html(this.count);
									 $('div#chat-box-'+this.sender+' .chat-box-header span.tinyMsgCount').show();
									 ...
									 /*if ($(".chat-group").find('span[rel="'+from+'"]').is(':empty') &&  (!soundCount) ){
            ...
            if(buddy.more){
             $('ul#chat_box_body_id-'+user).append('<li id="load-more-wrap" style="text-align:center"><a onclick="javascript: load_thread(\''+buddy.id+'\', \''+buddy.limit+'\')" class="btn btn-xs btn-info" style="width:100%">View older messsages('+buddy.remaining+')</a></li>');
                ...
                if(buddy.scroll){
					...
					$('div#chat_box_body_id-'+user).slimscroll({ scrollBy: '400px' });
                }
